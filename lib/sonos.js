...
Sonos.prototype.request = function(endpoint, action, body, responseTag, callback) {
  ...
  var d;
  if (!!this.options.debug) d = withinEnvelope(body);
  ...
  request({
    ...
    if ((!!d) && (res.statusCode === 500)) {
      console.log("http://" + this.host + ":" + this.port + endpoint);
      console.log(d);
      console.log(body);
    }
 ...
 */
 Sonos.prototype.currentTrack = function(callback) {
  ...
  return this.request(_this.options.endpoints.transport, action, body, responseTag, function(err, data) {
    if (err) return callback(err);
 ...
 * @param  {Object}   metadata Optional - DIDL
 * @param  {Function} callback (err, playing)
 */
Sonos.prototype.play = function(uri, metadata, callback) {
  debug('Sonos.play(%j, %j, %j)', uri, metadata, callback);
  var action, body;
  var self = this;
  ...
  if ((callback !== null) && (uri !== null)) {
    return this.queueNext(uri, function(err, metadata, queued) {/* jshint unused: false */
      if (err) {
        return callback(err);
      }
 ...
 */
Sonos.prototype.queueNext = function(uri, metadata, callback) {
  debug('Sonos.queueNext(%j, %j, %j)', uri, metadata, callback);
  if ((typeof metadata === 'function') && (typeof callback === 'undefined')) {
    callback = metadata;
    metadata = '';
  }
...
Sonos.prototype.setPlayMode = function(playmode, callback) {
  ...
  var body = '<u:SetPlayMode xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><DesiredPlayMode>' + mode + '</DesiredPlayMode><NewPlayMode>' + mode + '</NewPlayMode></u:SetPlayMode>';
 ...
 * @param  {Function} callback (err, info)
 */
 Sonos.prototype.getTransportInfo = function(callback) {
  debug('Sonos.getTransportInfo(' + ((callback) ? 'callback' : '') + ')');
  ...
  var responseTag = 'u:GetTransportInfoResponse';

  return this.request(this.options.endpoints.transport, action, body, responseTag, function(err, data) {
    if (err) return callback(err);

// state: STOPPED, PAUSED_PLAYBACK, PLAYING, TRANSITIONING, NO_MEDIA_PRESENT
    callback(null, { state  : data[0].CurrentTransportState[0]
                   , status : data[0].CurrentTransportStatus[0]
                   , speed  : data[0].CurrentSpeed[0]
                   });
  });
};
